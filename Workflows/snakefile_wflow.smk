...
rule all_wflow:
    ...
    input:
        expand(join(root_dir, "03_Runs", "{region}", "{runname}", "wflow", "event_precip_{forcing}", "events", "run_default", "output_scalar.nc"), zip, region=regions, runname=runname_ids, forcing=forcing),
...
rule update_forcing_wflow_warmup:
    input: 
        ...
        staticmaps = join(root_dir, "02_Models", "{region}", "{runname}", "wflow", 'staticmaps.nc'), 
    output:
        join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "warmup", "inmaps.nc"),
        join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "warmup", "wflow_sbm.toml"),
    params:
        wflow_root_noforcing = join(root_dir, "02_Models", "{region}", "{runname}", "wflow"),
        wflow_root_forcing= join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}"),
        start_time = get_start_time,
        end_time = get_end_time,
        data_cat = get_datacatalog,
    script:
        join(curdir, "scripts", "model_building", "wflow", "update_forcing_wflow_warmup.py")
...
rule update_forcing_wflow_event:
    input: 
        ...
        staticmaps = join(root_dir, "02_Models", "{region}", "{runname}", "wflow", 'staticmaps.nc'), 
    output:
        join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "events", "inmaps.nc"),
        join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "events", "wflow_sbm.toml"),
    params:
        wflow_root_noforcing = directory(join(root_dir, "02_Models", "{region}", "{runname}", "wflow")),
        wflow_root_forcing= directory(join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}")),
        start_time = get_start_time,
        end_time = get_end_time,
        forcing = "{forcing}",
        data_cat = get_datacatalog,
    script:
        join(curdir, "scripts", "model_building", "wflow", "update_forcing_wflow_event.py")

rule run_wflow_warmup:
    input:
        join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "warmup", "inmaps.nc"),
        toml = join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "warmup", "wflow_sbm.toml"),
    output:
        join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "events", "instate", "instates.nc"),
    params:
...
rule run_wflow_event:
    input:
        join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "events", "instate", "instates.nc"),
        join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "events", "inmaps.nc"),
        toml = join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "events", "wflow_sbm.toml"),
    output:
        join(root_dir, "03_Runs", "{region}", "{runname}", "wflow","event_precip_{forcing}", "events", "run_default", "output_scalar.nc"),
    params:
        exe = join(root_dir, "02_Models", "00_executables", "wflow0.8.1", "wflow_cli", "bin", "wflow_cli.exe"),
        julia_env_fn = "~/.julia/environments/v1.9",
    shell:
