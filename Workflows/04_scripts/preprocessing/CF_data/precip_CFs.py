...
if "snakemake" in locals():
    ...
    start_date = "20220410 000000"  # Start time of the SFINCS model run in format: YYYYMMDD HHMMSS
    end_date = "20220413 000000"
    wflow_region = (
        "/p/11210471-001-compass/02_Models/durban/Durban2022/sfincs/gis/region.geojson"
    )
    data_cat = [
    ...
    CF_value = -10  # -10% thermodynamic delta (ClimateDT large-box attribution)
...
if CF_dataset_name not in CF_datacatalog:
    ...
    if all(
        hasattr(CF_datacatalog[CF_dataset_name], attr)
        for attr in ["rename", "unit_mult", "unit_add"]
    ):
    ...
    if hasattr(CF_datacatalog[CF_dataset_name], "meta") and isinstance(
        CF_datacatalog[CF_dataset_name].meta, dict
    ):
        # Remove all metadata except for 'notes' and add CF info
        notes = CF_datacatalog[CF_dataset_name].meta.pop(
            "notes", None
        )  # Save the current 'notes' if exists
        CF_datacatalog[CF_dataset_name].meta.clear()

        if notes is not None:
            CF_datacatalog[CF_dataset_name].meta[
                "notes"
            ] = f"Copied {precip_name} dataset and adjusted with CF value {CF_value_txt}%"
