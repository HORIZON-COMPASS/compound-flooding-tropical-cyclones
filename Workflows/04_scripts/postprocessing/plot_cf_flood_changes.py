...
percent_diff = xr.where(zsmax_cf0 > 0, (diff / zsmax_cf0) * 100.0, np.nan)

# Flood extent (area) change (using same threshold as plots)
flood_threshold = 0.0  # meters
flood_mask_cf0 = zsmax_cf0 > 0 # removed > flood_treshold
flood_mask_cf8 = zsmax_cf8 > 0 # removed > flood_treshold
try:
...
extent_area_pct = (
    (extent_area_diff_m2 / area_cf0_m2 * 100.0) if area_cf8_m2 > 0 else np.nan # changes / CF to / F
)
...
if not np.isnan(cell_area_m2):
    ...
    volume_pct = (
        (volume_diff_m3 / volume_cf0_m3 * 100.0) if volume_cf8_m3 > 0 else np.nan
    )
else:
    volume_cf0_m3 = volume_cf8_m3 = volume_diff_m3 = volume_pct = np.nan

# Only show areas with positive water depth (above ground)
zsmax_cf0_plot = zsmax_cf0.where(zsmax_cf0 > 0.0)
zsmax_cf8_plot = zsmax_cf8.where(zsmax_cf8 > 0.0)
