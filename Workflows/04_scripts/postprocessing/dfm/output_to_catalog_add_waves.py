...
station_to_plot = ds_combined["match"].values[79]  # pick station
...
plt.title(f"Water levels over time for station {station_to_plot} with max of {(ds_combined["wave_setup"].max(dim='time').values[79]):.2f} m")
plt.legend()
plt.show()
...
# Coordinate transformer: web mercator to lat/lon
...
# Filter gdf_wave geometries inside those limits
df_wave_cut = df_wave[
...
# ------------------------------------------------------------------
# BAR CHART 1: Absolute difference for original datasets
# ------------------------------------------------------------------
ax_abs = fig.add_subplot(gs[0, 1])

# --- Bar plot layout settings ---
lat = ds_filtered.lat.values
...
ax_abs.barh(gdf_dfm_matched['lat'], gdf_dfm_matched["wl_max_component"],
            height=0.005, 
            color="steelblue", label="Tide + Surge")

# Plot swash_IG only where positive (on top of setup)
ax_abs.barh(gdf_dfm_matched['lat'], 
            gdf_dfm_matched["wave_max_component"],
            left=gdf_dfm_matched["wl_max_component"],
            height=0.005, 
            color="orange", label="Wave Setup")
...
# ------------------------------------------------------------------
# BAR CHART 2: Absolute difference for combined dataset to check
# NOTE: Max wave and max tide+sirge do not happen at the same time
# ------------------------------------------------------------------
ax_abs = fig.add_subplot(gs[0, 2])

# DFM DataArray: extract coords and values
df_combined = pd.DataFrame({
    'lon': ds_combined.lon.values,
    'lat': ds_combined.lat.values,
    'wl_max_component': ds_combined["waterlevel"].max(dim='time').values,  # or any stat
    'wave_max_component': ds_combined["wave_setup"].max(dim='time').values,  # or any stat
    'tide_surge_max_component': ds_combined["tide_surge"].max(dim='time').values
})

# Plot max water level from tide+surge, waves and combined (wave setup on top of tide+surge)
ax_abs.barh(df_combined['lat'], 
            df_combined["wave_max_component"],
            left=df_combined["tide_surge_max_component"],
            height=0.005, 
            color="orange", label="Wave Setup (on top of T+S)")
...
ax_abs.barh(df_combined['lat'], df_combined["wl_max_component"],
            height=0.005, 
            color="green", label="Total waterlevel")
