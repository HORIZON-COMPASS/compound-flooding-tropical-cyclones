...
population_raster_path = "/p/11210471-001-compass/01_Data/population_data/Worldpop/moz_ppp_2020_UNadj_constrained.tif"
...
try:
    ...
    if "total_damage" in buildings_fp.columns:
        ...
        print(
            f"Filtered buildings with damage > 0: {filtered_count} out of {original_count}"
        )
...
if not Path(population_raster_path).exists():
    ...
    print(
        "Please update the population_raster_path variable with the correct path to your population raster."
    )
    exit(1)
...
with rasterio.open(population_raster_path) as src:
    ...
    batch_size = 1000
    total_batches = len(cell_rows) // batch_size + (
        1 if len(cell_rows) % batch_size else 0
    )
    ...
    for batch_idx in range(total_batches):
        ...
        print(
            f"Processing batch {batch_idx + 1}/{total_batches} (cells {start_idx}-{end_idx})..."
        )
        ...
        buildings_in_cells = gpd.sjoin(
            result_df, cells_gdf, how="inner", predicate="intersects"
        )
        ...
        if not buildings_in_cells.empty:
            ...
            for cell_id, cell_buildings in cell_groups:
                ...
                if total_area_in_cell > 0:
                    ...
                    result_df.loc[
                        building_indices, "population"
                    ] += pop_allocation.values
...
building_keys = [
    ...
    if "object" in col.lower() or col.lower() in ["id", "fid"]
]
exposure_keys = [
    col
...
if common_key:
    print(
        f"Using common key: {common_key[0]} (buildings) -> {common_key[1]} (exposure)"
    )
    ...
    if "relative_damage" in exposure_with_updates.columns:
        exposure_with_updates["relative_damage"] = exposure_with_updates[
            "relative_damage"
        ].fillna(0)
    ...
    if len(exposure_df) == len(result_df):
        ...
        if "relative_damage" in result_df.columns:
            exposure_with_updates["relative_damage"] = result_df[
                "relative_damage"
            ].values
    else:
...
total_population_raster = (
    valid_cells.sum()
)  # This was calculated from clipped_data above
total_population_allocated = result_df["population"].sum()
ratio = (
    total_population_allocated / total_population_raster
    if total_population_raster > 0
    else 0
)
...
if 0.9 <= ratio <= 1.1:
    print(
        "Population sanity check PASSED: Population allocation is within acceptable range"
    )
else:
    print(
        "Population sanity check WARNING: Population allocation differs from raster total."
    )
    print(
        "This is expected when using pre-filtered buildings (spatial.fgb) that only include flood-affected areas."
    )
...
print(
    f"Buildings with both population and flood depth > 0: {len(result_df[(result_df['population'] > 0) & (result_df['inun_depth'] > 0)])}"
)
...
print(
    f"Average flood depth per building (where > 0): {result_df[result_df['inun_depth'] > 0]['inun_depth'].mean():.2f} m"
)
