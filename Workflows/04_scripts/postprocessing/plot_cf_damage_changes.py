...
print(
    f"Sample coordinates CF0: lon={gdf_cf0['x'].iloc[0]:.4f}°, lat={gdf_cf0['y'].iloc[0]:.4f}°"
)
print(
    f"Sample coordinates CF-8: lon={gdf_cf8['x'].iloc[0]:.4f}°, lat={gdf_cf8['y'].iloc[0]:.4f}°"
)
...
print(
    merged[
        [
            ...
            "x",
            "y",
            f"{DAMAGE_COLUMN}_cf0",
            f"{DAMAGE_COLUMN}_cf8",
            "damage_diff",
        ]
    ].head()
)
...
if DAMAGE_COLUMN == "relative_damage":
    ...
    print(
        f"Percentage change: {((cf0_damage.sum() - cf8_damage.sum()) / cf8_damage.sum() * 100):.1f}%"
    )
...
plt.tight_layout()
plt.suptitle(
    f"Economic Damage Analysis - {EVENT_NAME}: Factual vs Counterfactual ({DAMAGE_COLUMN})",
    fontsize=16,
    fontweight="bold",
    y=1.02,
)
...
# Save the figure
output_file_main = (
    OUTPUT_DIR / f"damage_{EVENT_NAME.lower()}_{DAMAGE_COLUMN}_comparison.png"
)
...
if DAMAGE_COLUMN == "total_damage":
    ...
    line_x_position = (
        bar_positions[1] + (bar_positions[1] - bar_positions[0]) * 0.3
    )  # Position to the right of the rightmost bar
    line_extension = (
        bar_positions[1] - bar_positions[0]
    ) * 0.04  # Small horizontal ticks
    ...
    text_x = (
        line_x_position + (bar_positions[1] - bar_positions[0]) * 0.1
    )  # Position to the right of the line
    text_y = (
        (bar_heights[lower_bar_idx] + bar_heights[higher_bar_idx]) / 2
    ) * 0.97  # Center vertically on the line
    ...
    ax_bar.text(
        text_x, text_y, diff_text, ha="left", va="center", fontsize=10, rotation=0
    )
    # Use a small vertical offset for the second text line
    text_offset = (
        bar_heights[higher_bar_idx] - bar_heights[lower_bar_idx]
    ) * 0.35  # Smaller vertical offset
    ax_bar.text(
        text_x,
        text_y + text_offset,
        "Climate change \n attribution:",
    ...
    # Save the bar chart
    output_file_bar = (
        OUTPUT_DIR / f"damage_{EVENT_NAME.lower()}_{DAMAGE_COLUMN}_totals_barchart.png"
    )
...
# Title
ax.set_title(
    f"Economic Damage Changes - {EVENT_NAME}: Factual vs Counterfactual\n(Red = Higher damage, Blue = Lower damage)",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
...
# Save
output_file_diff = (
    OUTPUT_DIR / f"damage_{EVENT_NAME.lower()}_{DAMAGE_COLUMN}_difference_only.png"
)
...
ax_factual.set_title(
    f"Economic Damage - {EVENT_NAME}: Factual Scenario\n({DAMAGE_COLUMN})",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
...
output_file_factual = (
    OUTPUT_DIR / f"damage_{EVENT_NAME.lower()}_{DAMAGE_COLUMN}_factual_only.png"
)
plt.savefig(output_file_factual, dpi=300, bbox_inches="tight")
plt.show()

# ===== INDIVIDUAL COUNTERFACTUAL PLOT =====
print("Creating counterfactual-only damage plot...")
...
# Title
ax_cf.set_title(
    f"Economic Damage - {EVENT_NAME}: Counterfactual Scenario\n({DAMAGE_COLUMN})",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
...
output_file_counterfactual = (
    OUTPUT_DIR / f"damage_{EVENT_NAME.lower()}_{DAMAGE_COLUMN}_counterfactual_only.png"
)
...
if DAMAGE_COLUMN == "relative_damage":
    ...
    print(
        f"Buildings with >${threshold_low:.0f} damage increase: {(damage_diff > threshold_low).sum()}"
    )
    print(
        f"Buildings with >${threshold_low:.0f} damage decrease: {(damage_diff < -threshold_low).sum()}"
    )
    print(
        f"Buildings with >${threshold_high:.0f} damage increase: {(damage_diff > threshold_high).sum()}"
    )
    print(
        f"Buildings with >${threshold_high:.0f} damage decrease: {(damage_diff < -threshold_high).sum()}"
    )

# ===== EXPORT SUMMARY DATA TO CSV =====
print("\nExporting summary data to CSV...")

# Calculate total damage for each scenario
total_cf0 = cf0_damage.sum()
total_cf8 = cf8_damage.sum()
...
summary_data = pd.DataFrame(
    {
        ...
        "Damage_Type": [DAMAGE_COLUMN, DAMAGE_COLUMN, DAMAGE_COLUMN, DAMAGE_COLUMN],
...
# Save to CSV
csv_output_file = (
    OUTPUT_DIR / f"damage_{EVENT_NAME.lower()}_{DAMAGE_COLUMN}_summary.csv"
)
...
if DAMAGE_COLUMN == "relative_damage":
    ...
    print(
        f"Buildings with damage in CF-8: {(merged[f'{DAMAGE_COLUMN}_cf8'] > 0).sum()}"
    )
else:
    print(f"Buildings with damage in CF0: {(merged[f'{DAMAGE_COLUMN}_cf0'] > 0).sum()}")
    print(
