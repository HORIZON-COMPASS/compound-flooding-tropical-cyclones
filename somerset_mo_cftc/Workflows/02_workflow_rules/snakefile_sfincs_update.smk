...
def get_datacatalog(wildcards):
    if os.name == 'nt': #Running on windows
        ...
        return [
            ...
            # join(curdir, '..', "03_data_catalogs", "data_catalog_static.yml"),
...
#        batchfile = "{dir_run}"+"/sfincs_"+"{runname}"+"/run_sfincs.bat"
    ...
    run:
        ...
        if os.name == 'posix': 
            ...
            shell("mv sfincs-cpu_latest.sif {params.dir_run_with_forcing}")
            # shell("pwd", cwd=params.dir_run_with_forcing)
            shell("apptainer run sfincs-cpu_latest.sif", cwd=params.dir_run_with_forcing)
            # shell("echo {params.dir_run_with_forcing}")
            # shell("exit 1")
